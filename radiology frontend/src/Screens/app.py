...
model_path = os.path.join("", "model_better.keras")  # Update this path to your model file
...
def save_results(image, y_pred, save_image_path, mask=None):
    ...
    # Resize the blended image back to 256x256
    blended_resized = cv2.resize(blended, (256, 256))
...
def upload_images():
    ...
    """ Load the model """
    with CustomObjectScope({"dice_coef": dice_coef, "dice_loss": dice_loss}):
        model = tf.keras.models.load_model("model_better.keras")
